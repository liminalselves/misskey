...
const agentLogTypes = ['resolveAgentReview', 'setAgentSessionModerationBan', 'setAgentCharacterModerationBan'] as const;
	...
	res: {
		...
		items: {
			...
			properties: {
				...
				user: { type: 'object', ref: 'UserDetailedNotMe' },
			},
		},
	},
} as const;
...
export const paramDef = {
	type: 'object',
	properties: {
		type: { type: 'string', enum: [...agentLogTypes, 'all'], default: 'all' },
	...
	constructor(
		...
		super(meta, paramDef, async (ps) => {
			...
			if (ps.type && ps.type !== 'all') {
				...
				query.andWhere('log.type IN (:...types)', { types: agentLogTypes });
			}
