...
[#misskey](https://misskey.liminalselves.top/tags/misskey)

新增
- 智能体表情包：用户与智能体均可在会话中发送全站自定义表情（`:名字:` 行内渲染为图片，输入框表情按钮即发送入口）。
- 表情包描述体系：全站表情新增「智能体描述」字段（默认空，仅填写描述的表情进入智能体可用列表并注入提示词）；管理端表情编辑支持单个/批量 AI 生成描述，费用按识图模型单价扣调用者余额，批量后台串行执行并轮询进度。
- 角色专属表情包：创作者可为角色配置最多 50 个表情（任意比例图片/动图 + 必填描述），智能体依描述在合适时机以 `[[agent_sticker key=...]]` 发送，气泡内按原始比例有界渲染；描述支持 AI 生成（扣创作者余额）；随发布快照进入审核，审核详情展示表情网格。
- 管理面板「智能体 → 表情包」新页签：启用开关、每轮表情包合计上限（默认 3，超出自动剥离）、全站表情注入上限（默认 200）。
- 用户消息中的 `:名字:` 发往模型时自动转义为对应描述，便于模型理解；助手回复超限或编造的表情标签在入库前移除；主动消息管线同规则。

优化
- 会话导出/导入零格式变更：表情语法即消息文本，v1-v5 与旧上下文文件全部向后兼容。
- 自定义表情符号（beta）表格新增「智能体描述」列（可直接编辑、随批量更新提交），全部列标题与搜索/排序控件本地化（此前为硬编码英文）。

## 2026.2.0-liminalselves.2

[#misskey](https://misskey.liminalselves.top/tags/misskey)

新增
- 每日签到：签到可获得加权随机基础额度，并叠加连续签到、身份组与节日倍率；签到成功弹窗展示完整计算公式，签到页提供日历与连续统计。
- 补签：支持消耗额度补签近期遗漏日期，单月补签成本随次数递增；补签记录在日历中标注，消费日志区分「签到奖励」与「补签消耗」。
- 管理后台新增签到报表与「撤销当日签到」测试功能；签到倍率配置由智能体设置迁移至角色管理（按角色配置、取最高倍率生效）。
- 智能体会话被管理员封禁后，用户进入该会话将看到专用处置页面，展示封禁原因，并仅保留「导出会话」与「删除会话」两个入口。
- 会话导入导出升级至 v3：导出新增识图模型与定时主动消息计划，导入向后兼容 v1/v2/v3 并对未知字段安全忽略。
- 卡密购买链接：管理面板「额度」标签页可配置购买入口，用户在「我的用量」兑换区看到跳转链接（未配置则不显示）。
- 对话模型卡片显示「今日免费：已用 X / 共 Y 次」每日免费额度信息。

优化
- 消费日志与用量统计：每日免费额度调用按持久化快照显示「免费次数 X/Y」；签到/补签流水保留在消费日志但不计入「费用」汇总；模型用量分析排除空模型记录、免费次数改为近 30 天统计、模型名映射含已下架模型；余额显示小数。
- 对话风格空状态改为详细引导：说明对话风格作用、两种获取途径，并提供直达风格广场的按钮。
- 签到弹窗、待生图卡片等多处文案口语化，去除表情符号堆砌与 AI 化措辞。

修复
- 修复移动端「发送消息前请先选择对话风格」提示被截断的问题，窄屏下完整换行显示。
- 修复撤销当日签到后用户无法重新签到的问题（移除本地缓存前置判断，改由后端幂等校验）。
- 修复补签失败时错误提示为英文泛化文案的问题，改为展示具体中文原因。
- 修复 API 错误信息含中文时写入 WWW-Authenticate 响应头触发 ERR_INVALID_CHAR 的问题。
- 修复 i18n 构建在临时文件残留或 esbuild 报错时崩溃的问题。
- 修复签到弹窗组件因模板重复拼接导致无法加载的问题。
- 修复管理面板「额度」标签页缺少保存栏、卡密购买链接无法保存的问题。

部署须按时间戳顺序执行迁移：`1773000000000-AgentCompressionStickyUniqueFingerprint`、`1775700000000-AgentCompressionStickyRetryCount`、`1775800000000-AgentCheckin`、`1775900000000-RoleCheckinMultiplier`、`1776000000000-AgentRedeemPurchaseUrl`、`1776100000000-AgentSessionModerationBannedReason`。

## 2026.2.0-liminalselves.1

[#misskey](https://misskey.liminalselves.top/tags/misskey)

更新
- 智能体治理工作台重新上线：整合待处理审核、内容库、会话治理、外审拦截、AI 生图和操作日志，管理入口统一为“智能体治理”。
- 智能体会话导入能力增强，长对话最多支持导入 10000 条消息，并加入分批处理与上限校验以降低大文件导入时的压力。
- 用户冻结新增面向用户的冻结原因与解封时间展示；被冻结用户会自动退出所有端，重新登录时可看到冻结原因和解封时间。
- 智能体图片生成模型选择改为卡片式展示，直接显示费用、状态和成功率等关键信息。

优化
- 优化 `/chat` 打开速度，为大表历史消息查询补充更适合的查询路径与索引，降低生产环境聊天首页加载时间。
- 优化智能体聊天、广场、我的用量、治理工作台等页面的导航、布局、移动端适配和长文本显示。
- 优化智能体聊天页顶部导航顺序，按消息、搜索、模型、生图、记忆、世界书、对话风格、会话排列。
- 优化全站 CAPTCHA 展示样式，并在登录密码验证流程加入人机验证以降低暴力破解风险。
- 优化前端兼容性，降低部分构建目标要求，并补充旧浏览器环境所需的基础兼容处理。
- 优化帖子详情与讨论阅读：帖子卡片直接进入主详情页，默认加载回复；子回复保留首批 10 条分支展开，并将正文与表情互动左右分离，减少评论区冗余操作栏。
- 优化帖子详情导航：新增页内返回入口，直接打开帖子时可回到首页，避免用户只能依赖浏览器后退。

调整
- 智能体广场中的“预设广场”统一改名为“对话风格”。
- 我的用量记录中，模型列不再重复显示“对话 / 生图”类型前缀。
- README 已更新为阈界人格项目组维护的第三方 Misskey fork 说明，并指向新的组织仓库与官网。
- 删除旧的智能体聊天审计和图片审查独立页面入口，统一由智能体治理工作台承接。

修复
- 修复智能体页面在广场、创作、我的用量之间切换时，需要点击两次才真正切换的问题。
- 修复忘记密码时可输入与用户名不匹配的邮箱并继续发送验证的问题。
- 修复智能体聊天页中普通用户查看他人公开智能体时，头像因文件权限限制回退为默认机器人图标的问题。
- 修复智能体消息发送在客户端切后台后可能重复发送的问题，并加入客户端请求去重。
- 修复智能体图片预览下载链接在对象存储上因响应头覆盖导致下载失败的问题。
- 修复智能体治理工作台中继续载入按钮、消息检索、重置、导航排版和列表滚动等交互问题。
- 修复右下角“已和服务器断开连接”提示在流连接恢复后仍持续显示的问题；断连超过 5 秒才显示提示。

## 2026.2.0

### 智能体功能全面更新上线

- Enhance: 智能体系统进入正式大版本：覆盖角色与对话风格广场、创建与编辑、版本管理、审核治理、管理后台、用量与计费、兑换码、图片生成、长期记忆、上下文压缩、世界书匹配、外部审计等完整链路。
- Enhance: 智能体会话页新增「会话操作」子页面，集中管理会话重命名、删除、上下文边界定位，以及会话导入导出等操作。
...
- Enhance: 发现模块（Discovery/Featured）推流机制改造
  ...
  - 实现伪无限滚动，通过 `excludeIds` 参数去重已展示内容；`notes/featured` 在单次响应内会对被规则过滤掉的候选继续向后补足，直至凑满 `limit` 或耗尽排名池，避免未返回的帖子 ID 无法进入前端排除列表而引发翻页重复
...
- Enhance: 私信搜索结果支持点击定位功能
  ...
  - 目标消息会短暂高亮显示以便识别
  - 支持从私信首页搜索和聊天室内搜索两种场景
  - 优化上下文加载策略，支持双向分页与精准定位
  - 修复加载历史消息时的排序错误及虚假新消息提示
- Enhance: 智能体会话界面（Chat）改版
...
- Enhance: 发现页热门板块排序选项使用专用文案键 `exploreFeaturedSortLatest`（简体中文等为「最新」），与「推荐」「热度」等两字标签对称；其它仍依赖「新→旧」语义的界面继续使用 `_order.newest`，互不影响
- Enhance: 路由与主导航：已登录用户访问 `/` 进入发现页（Discovery），个人时间线为 `/timeline`；`/explore` 重定向至 `/`；侧栏与移动端底栏「首页」等与发现入口对齐
...
- Fix: 修复私信未读状态在阅读消息后返回列表页时仍显示小光点的问题
  - 新增 `chatRead` 事件用于实时同步已读状态
  - 优化前后端事件通信时序，确保 API 返回的数据与 Redis 状态一致
- Enhance: 实现类似 QQ/Telegram 的私信引用回复功能
  - 右键消息菜单选择「引用」即可快速回复特定消息
  - 输入框上方显示引用消息预览，支持一键取消引用
